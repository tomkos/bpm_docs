...
|---------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|                     | execute() 								 Creates a Box collaboration folder which requires the p8 user to log into Box and adds   the logged in user as a collaborator on the Box folder. |
|                     | getBoxUserId(repository) 								 Returns the logged in Box user from the specified repository.                                                                     |
|                     | isEnabled() 								 Enable this action if Box collaboration is not already created and p8 user has write access to the Case.                                       |
|                     | isVisible() 								 This action is visible if Box collaboration is enabled, hidden if not.                                                                         |
|                     | showDialog(caseModel, folderName, folderName) 								 Shows the dialog to create a Box folder.                                                                     |