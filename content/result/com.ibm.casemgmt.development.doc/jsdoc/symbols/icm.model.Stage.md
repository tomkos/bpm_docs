...
Defined in:  <icm\model\Stage.js>.

| Constructor Attributes   | Constructor Name and Description                                                                                                                                |
|--------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------|
|                          | icm.model.Stage() 							 A Stage object model that is constructed internally by the model API when stages are retrieved from the IBM  Case Manager API plugin. |

| Field Attributes   | Field Name and Description                                                                           |
|--------------------|------------------------------------------------------------------------------------------------------|
|                    | caseId  The parent case ID of this stage.                                                            |
|                    | completedDate  The date the stage was completed.                                                     |
|                    | description  The textual description of the stage.                                                   |
|                    | dueDate  The stage due date when the stage is in a working status.                                   |
|                    | duration  The stage duration.                                                                        |
|                    | durationType  The duration type if the stage has a duration: 1=hours, 2=days, 3=weeks.               |
|                    | estimatedCompleteDate  For a task that is WAITING, this is the esitmated date the task will complete |
|                    | estimatedStartDate  For a task that is WAITING, this is the estimated date the task will start       |
|                    | isOverdue  Indicator for whether the currently running or a completed stage is overdue               |
|                    | name  Stage display name.                                                                            |
|                    | overdueTime  The overdue time on the currently running or a completed stage.                         |
|                    | repositoryId  The ID of the repository that contains this stage.                                     |
|                    | restartCount  The restart count for the stage.                                                       |
|                    | startedDate  The date the task was started.                                                          |
|                    | status  The stage status: completed, working, onhold or waiting.                                     |
|                    | symbolicName  Stage unique symbolic name.                                                            |
|                    | timeSpent  The total time that has been spent on the currently running stage.                        |

| Method Attributes   | Method Name and Description                                                                   |
|---------------------|-----------------------------------------------------------------------------------------------|
|                     | getDurationInMin() 								 Returns the expected duration in minutes.                         |
|                     | getDurationType() 								 Returns the duration type as a string.                             |
|                     | getOverdueString() 								 Returns the amount of time that the stage is overdue as a string. |
|                     | getProgressString() 								 Returns the time spent in this stage as a string.                |

The time spent is calculated based on when the stage started until now.  If the stage has
been restarted one or more times, the time spent while the stage was previously running
is also taken into account.  Any time that the stage was on hold is not counted against the total time spent.

Examples

(timeSpent > duration)

Duration 3 hours, timeSpent 4.223.
Returns 4.2 hours

(timeSpent < duration)

Duration 1 week, timeSpent 0.3.
Then 7 days * 0.3. Returns 2.1 days

Duration 1 week, timeSpent 0.1.
7 days * 0.1 = 0.7 day. Returns 16.8 hours

Duration 1 week, timeSpent 0.01
7 days * 0.001 = 0.007 days = 0.168 hours = 10.8 minutes. Returns floored 10 minutes