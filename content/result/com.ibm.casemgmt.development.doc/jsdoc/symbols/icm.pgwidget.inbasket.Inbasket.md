...
# Class icm.pgwidget.inbasket.Inbasket

Extends
					icm.base.BasePageWidget, icm.base.BaseActionContext.
...
Defined in:  <icm\pgwidget\inbasket\Inbasket.js>.
...
|---------------------|-----------------------------------------------------------------------------------------------------------|
|                     | constructor()                                                                                             |
|                     | getColumnDecorator() 								 Method to provide column decorators for columns in Inbasket.                |
|                     | handleApplyFilter(payload) 								 Handler for icm.ApplyFilter event                                     |
|                     | handleClearContent(payload) 								 Handler for icm.ClearContent event                                   |
|                     | handleICM\_ApplyFilterEvent(payload) 								 Wrapper Handler for icm.ApplyFilter event                    |
|                     | handleICM\_ClearContentEvent(payload) 								 Wrapper Handler for icm.ClearContent event                  |
|                     | handleICM\_ReceiveInbasketEvent(payload) 								 Wrapper Handler for icm.ReceiveInbasket event            |
|                     | handleICM\_ReceiveRoleEvent(payload) 								 Wrapper Handler for icm.ReceiveRole event                    |
|                     | handleICM\_RefreshEvent(payload) 								 Wrapper Handler for icm.Refresh event                            |
|                     | handleICM\_RequestOpenWorkItemEvent(payload) 								 Wrapper Handler for icm.RequestOpenWorkItem event    |
|                     | handleICM\_RequestSelectWorkItemsEvent(payload) 								 Wrapper Handler for icm.RequestSelectedRows event |
|                     | handleReceiveInbasket(payload) 								 Handler for icm.ReceiveInbasket event                             |
|                     | handleReceiveRole(payload) 								 Handler for icm.ReceiveRole event                                     |
|                     | handleRefresh(payload) 								 Handler for icm.Refresh event                                             |
|                     | handleRequestOpenWorkItem(payload) 								 Handler for icm.RequestOpenWorkItem event                     |
|                     | handleRequestSelectWorkItems(payload) 								 Handler for icm.RequestSelectedRows event                  |
|                     | postCreate()                                                                                              |
...
var payload = {"dynamicFilters": dynamicFilters, "cursorLocation": 1,"cleanDynamicFilterByReset":true}; //"cursorLocation" is an optional parameter specify the default cursor location.
```