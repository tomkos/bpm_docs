...
# Class icm.pgwidget.caseinfo.CaseInfo

Class that represent the CaseInfo widget
				
				
					
Defined in:  <icm\pgwidget\caseinfo\CaseInfo.js>.
...
|---------------------|---------------------------------------------------------------------------------------------------------------------------------|
|                     | addChild(dijitObj) 								 Add a new dijit as a tab in Case Information.                                                       |
|                     | handleICM\_ClearContentEvent() 								 Handle icm.ClearContent event Clear the content in the Case Information widget.          |
|                     | handleICM\_FilterHistoryEvent(historyFilter) 								 Handler for the icm.FilterHistory event.                                   |
|                     | handleICM\_RefreshTabEvent(payload) 								 Handler for the icm.RefreshTab event.                                               |
|                     | handleICM\_SelectCaseEvent(payload) 								 Handler for the icm.SelectCase event when selecting a case on case page.            |
|                     | handleICM\_SelectInitialFolderEvent(subFolderPath) 								 Handler for the icm.SelectInitialFolder event.                       |
|                     | handleICM\_SelectTabEvent(payload) 								 Handler for the icm.SelectTab event.                                                 |
|                     | handleICM\_SendCaseInfoEvent(payload) 								 Handler for the icm.SendCaseInfo event when opening a case on case detail page.   |
|                     | handleICM\_SendNewTaskInfoEvent(payload) 								 Handler for the icm.SendNewTaskInfo event when adding a task on Add Task page. |
...
- tabId: The ID of the tab that is selected.
...
- taskEditable: An icm.model.TaskEditable
