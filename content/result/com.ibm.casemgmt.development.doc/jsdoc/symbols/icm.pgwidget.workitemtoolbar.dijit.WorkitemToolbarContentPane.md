...
# Class icm.pgwidget.workitemtoolbar.dijit.WorkitemToolbarContentPane

Defined in:  <icm\pgwidget\workitemtoolbar\dijit\WorkitemToolbarContentPane.js>.
...
|---------------------|------------------------------------------------------------------------------------------------------------------------|
|                     | buildToolbar(toolbarAttachPoint) 								 build a Toolbar into the contentPane and destroying the existing one if have |
|                     | constructor()                                                                                                          |
|                     | destroy() 								 destroy in case memory leak                                                                         |
|                     | escapeInstruction(str)                                                                                                 |
|                     | onClickLearnMoreButton()                                                                                               |
|                     | postCreate()                                                                                                           |
|                     | updateHeaderInformation() 								 update the header content of content Pane according to work item information        |
|                     | updateViewPaneByModel(toolbarAttachPoint) 								 update the UI of content Pane according to model                    |

- toolbarAttachPoint: indicate which toolbar attachpoint should be used for getting the settings of actions;
