...
|---------------------|-----------------------------------------------------------------------------------------------------------------------------------|
|                     | constructor()                                                                                                                     |
|                     | handleICM\_ClearContentEvent(payload) 								 Handler for icm.ClearContent event Clear the content in the Process History widget. |
|                     | handleICM\_SendWorkItemEvent(payload) 								 Handler for the icm.SendWorkItem event when opening a work item page.               |
|                     | handleICM\_TaskSelectEvent(payload) 								 Handler for the icm.TaskSelect event when selecting a task on case detail page.       |
|                     | onSelectedItem(selectItem) 								 Event fired when an item in the grid is selected.                                             |
|                     | postCreate()                                                                                                                      |

- workItemEditable: An
           icm.model.WorkItemEditable object that represents
           the work item for which process history is to be
           displayed.

- task: An icm.model.TaskEditable object that
           represents the task for which process history is to be
           displayed.