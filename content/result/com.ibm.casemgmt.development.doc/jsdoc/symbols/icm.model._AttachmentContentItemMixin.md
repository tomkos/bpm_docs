...
This class is mixed in to the navigator ContentItem object.
       This class provides some ICM specific attributes on the ContentItem object.
...
|--------------------------|------------------------------------------------------------------------------------------------------|
|                          | icm.model.\_AttachmentContentItemMixin() 							 Constructs the attachment content item mixin object. |

| Field Attributes   | Field Name and Description                                                                                                                        |
|--------------------|---------------------------------------------------------------------------------------------------------------------------------------------------|
|                    | attachmentValueItem  Contains a reference to the javascript object from the property value used to retrieve or construct this ContentItem object. |
|                    | unretrievedAttachmentStatus  Indicates the reason a content item was not retrieved as part of an attachment.                                      |

This property is only meaningful when attachment content items are retrieved.  Content items obtained in the normal
manner, for example by browsing the folder hierarchy using the Content Navigagor API, may not have this property or
it may not be set to a value.  The caller in this case should assume the content item was retrieved from a repository
in the normal way and is equivalent to the status retrieved. 

retrieved.  The item was retrieved from the repository.  In most cases the individual ContentItem objects
    are retrieved in an efficient manner, for example by performing a search.  Only a sub-set of the attributes
    of a ContentItem may be retrieved initially.  Depending on the use case, it may be necessary to fully retrieve
    the ContentItem by calling retrieveAttributes().
external.  The item was not retrieved because it is stored in an external repository.
    The ContentItem in this case contains only
    a minimal subset of attributes taken from the attachment, for example id, name and repository.
               The caller can retrieve this item as needed using the minimal attributes in the ContentItem.