...
This interface defines symbolic constants that state whether the business flow manager database is used
 ...
 When the business flow manager database is used as an archive, only read access to stored objects is supported.
 All method calls that try to change the business flow manager archive are rejected. An
