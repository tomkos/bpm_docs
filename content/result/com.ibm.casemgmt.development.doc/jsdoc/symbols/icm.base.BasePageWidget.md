...
|                     | constructor()                                                                                                                  |
|                     | destroyRecursive()                                                                                                             |
|                     | getRole() 								 Gets the role context object that is injected by the IBM Case Manager Container widget at run time.         |
|                     | getSolution() 								 Gets the solution context object that is injected by the IBM Case Manager Container widget at run time. |
|                     | getWidgetAttributes() 								 Returns the edit setting attributes for a widget.                                               |
|                     | setDirtyState(state) 								 Notifies the page widget if the page contains unsaved changes.                                   |