...
Provides the results of a query request.
 ...
 The number of tuples returned is restricted by the threshold specified in the query request.
 If a threshold is not specified, all the qualifying tuples are returned.
 ...
 A tuple of the query result set is composed of the selected attributes of entities referenced
 directly or indirectly by work items, such as associated activity instances and process instances.
...
Returns:The type of the column values.
    Refer to QueryColumnInfo for the type definitions.
    - getBoolean
