...
3. Double-click the primitive to open the XML mapping wizard.
4. To enable high performance XML mapping, enable the Enable
high performance XML mapping using BO map execution check
box.
5. Click Next.
6 For both the input and output messages, identify the part of themessage that is available to the transformation by selecting an optionin the Message Root list:
    - / to transform the complete message. To
map SOAP attachments, use / as the root.
    - /body to transform the message body
    - /headers to transform the message headers
    - /context  to transform the message context
7. If your Mapping transformation primitive has both input and output
...
the list.
8. Select an operation, the message category and finally the message
type. Click OK to return to the New XML Mapping
wizard
9. Click Finish to launch the XML map editor.
10. In the editor, the source message is on the left and the target
