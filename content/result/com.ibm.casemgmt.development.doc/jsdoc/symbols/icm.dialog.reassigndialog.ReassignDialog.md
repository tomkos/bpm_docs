...
# Class icm.dialog.reassigndialog.ReassignDialog
...
Defined in:  <icm\dialog\reassigndialog\ReassignDialog.js>.
...
|---------------------|-------------------------------------------------------------------------------|
|                     | cleanLastSelection() 								 Clears the items that the user last selected.   |
|                     | constructor(args) 								 Constructs an instance of the Reassign dialog box. |
|                     | focus()                                                                       |
|                     | postCreate()                                                                  |
|                     | resize()                                                                      |
|                     | roleMemberResultSet()                                                         |
...
- targetOS: object store
- callback: A callback function that is to be called when the user clicks the OK button in the dialog box
- solution: The icm.model.Solution object that represents the current solution
- roles: An array of ecm.model.ProcessRole objects
- reassignItems: The array of work items that are to be reassigned
- allowRoleSearch: A flag that indicates whether the user can search by role
- allowDelegate: A flag that indicates whether the user can delegate work items

The following example shows how to construct the Reassign dialog box so that 
		  a user can reassign a work item by selecting a user from a role:
			...
			var dialog = new icm.dialog.reassigndialog.ReassignDialog({
					...
					restrictAssigment: restrictAssignment,
					restrictAssigmentRoles: restrictAssignmentRoles,
					callback:callback
				});
		  ...
		  The following example shows how to construct the Reassign dialog to prevent
		  a user from reassigning a work item by selecting a user from a role:
