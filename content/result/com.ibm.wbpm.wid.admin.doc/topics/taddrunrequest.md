...
1 Add a Verify Event step for a monitor request by completingthe following steps:
...
> Request. The Add Verify Request Event wizard opens.
    6. In the list box, expand the module, wire, and interface
and then select the operation that you want to use to verify the request
data that is passed to the target component of the wire.
    7. If you want to have variables automatically created
for the new Verify Request event, ensure that the Create
the variables for the new Verify Request events check
...
event: Request step is displayed under the associated
...
event: Request step is selected.
...
Request step. (If you need additional information about this property
...
correct.
4. In the Verify Request section,
ensure that one or more input variables are specified. You can accept
...
the variables for the new Verify Request events check
box in the Add Verify Request Event wizard, then you will need to
use the test data table to manually add one or more variables for
the Verify event: Request step. Information
about adding variables is found in the topic "Adding variables.")
5. In the test suite editor, press Ctrl-S to
save your changes.
3 Use the new Verify event: Request step to run a test bycompleting the following steps:
...
event: Request event in the Events area and view the test
results in the Detailed Properties area. (If you need additional information
about a property, position your cursor in the property and press F1 to
open the context-sensitive help information.)