...
Extends
					icm.base.BasePageWidget, icm.pgwidget.contentlist.dijit.ContentListContentPane.
...
|---------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|                     | handleICM\_ClearContentEvent() 								 Handler of icm.ClearContent event.                                                                                                            |
|                     | handleICM\_ReceiveDocumentIDsEvent(payload) 								 Display the list of documents that are referenced in the event payload.                                                          |
|                     | handleICM\_ReceiveSearchValuesEvent(payload) 								 Run the search with property values specified in the payload and display the list of documents that are returned by the search. |
|                     | handleICM\_ReceiveStoredSearchEvent(payload) 								 Run the stored search that is specified in the payload and display the list of documents that are returned by the search.       |
|                     | handleICM\_ReceiveWorkItemEvent(payload) 								 Handler for icm.ReceiveWorkItem event.                                                                                              |
|                     | handleICM\_RefreshContentEvent() 								 Handler for refresh event.                                                                                                                  |
...
payload = {"objectStoreNames" : ["TOS"],
							...
							"version": "current"  //released|all
				};
```

- name: Property symbolic name.
         value: Property value.

```
[
            {"name":"AT\_companyName",
             "value":"IBM"},
            {"name":"AT\_companyCode",
             "value":"987"}
         ]
```
