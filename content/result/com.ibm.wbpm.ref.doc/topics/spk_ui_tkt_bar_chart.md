...
In this example, the following configuration properties are set for Bar chart:

- In the behavior configuration properties:
    - A data service is specified.
    - Use X axis culling is selected.
    - Number of X axis ticks is set to 5.
    - Show tooltip is selected.
- In the appearance configuration properties:

- Data series color style is set to Primary.
- Show value labels is selected.
- Show breadcrumbs is selected.
- XY axis color style is set to Dark.
- Gridlines style is set to Dark.

All the other properties are left with their default settings.

These properties and their values result in a bar chart that looks like the following figure:
...
For detailed information on the available methods for Bar chart SDS, see the Bar chart SDS JavaScript API.
